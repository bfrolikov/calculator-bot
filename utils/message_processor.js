...
const getResult = (message) => {
  if (commandCheckers.checkHello(message))
    return 'Привет! Я Бот-Устный Счёт на службе ШКАФА. Я считаю ответы на задания устного счета для пропащих душ (учеников МВ). Напиши "Помощь", чтобы узнать, как мной пользоваться';
